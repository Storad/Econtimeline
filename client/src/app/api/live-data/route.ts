...
/**
 * On-demand live data API
 *
 * Fetches fresh data from FRED when users view the calendar.
...
async function fetchAllData(): Promise<Record<string, SeriesData>> {
  const results: Record<string, SeriesData> = {};

  // Fetch all series in parallel for speed
  const entries = Object.entries(EVENT_TO_SERIES);
  const promises = entries.map(async ([eventTitle, config]) => {
  ...
  for (const result of resolved) {
    if (result) {
      results[result.eventTitle] = result.data;
    }
  }
...
export async function GET() {
  ...
  if (dataCache && now - dataCache.timestamp < CACHE_TTL_MS) {
    return NextResponse.json({
      ...
      fetchedAt: new Date(dataCache.timestamp).toISOString(),
    });
  }
